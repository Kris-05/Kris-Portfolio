...
const Details = ({ position, company, time, work, link, grade }) => {
  const ref = useRef(null);

  return (
    <li
      ref={ref}
      className="my-2 md:my-8 md:ml-32 first:mt-0 last:mb-0 w-full mx-auto flex flex-col items-start justify-between"
    >
      <LiIcon />
      <motion.div
        initial={{ y: 50 }}
      ...
      >
        <h3 className="text-left w-full capitalize font-bold md:text-2xl">
          {position}&nbsp;
          <span className=" text-amber-400/75">@{company}</span>
        </h3>
        <div>
          <span span className="capitalize font-medium text-emerald-500/75">
            {time}
            {/* {time} | {address} */}
          </span>
          {/* Render grade if present */}
          {grade && (
            <span className="ml-2 font-medium text-amber-400/75">{grade}</span>
          )}
          ...
          {link && (
            <a
              ...
              className="ml-2 underline text-base inline-flex items-center"
            >
              <FaExternalLinkAlt className="text-sky-300"/>
            </a>
          )}
        </div>
...
const Card = ({ title, data, place }) => {
  ...
  return (
    <div>
      <div
        className={`bg-gray-800 rounded-3xl overflow-hidden relative z-0 p-8 
